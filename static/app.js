...
function ScoreDial({ score, size=84, strokeWidth=8, showLabel=true, breakdown=null }){
  ...
  const hasBreakdown = breakdown && Array.isArray(breakdown.components);
  const onEnter = (e) => {
  ...
  return (
    <div
      style={{position:'relative', width:size, height:size, cursor: hasBreakdown ? 'help' : 'default'}}
    ...
    >
      ...
      {hover && hasBreakdown && ReactDOM.createPortal(
        <ScoreBreakdownCard breakdown={breakdown} tierColor={tierColor} anchor={anchor}/>,
        document.body
      )}
    </div>
  );
...
function ScoreBreakdownCard({ breakdown, tierColor, anchor }){
  const comps = breakdown.components || [];
  const total = breakdown.total ?? comps.reduce((s,c)=>s+(c.points||0),0);
  const maxSum = comps.reduce((s,c)=>s+(c.max||0),0) || 100;
  const W = 280, PAD = 12;
  // Clamp so card stays inside the viewport. Anchor point is the dial's
  // bottom-right — card grows leftward from it.
  const left = Math.max(PAD, Math.min(anchor.x - W, window.innerWidth - W - PAD));
  const top = anchor.y + 8;
  return (
    <div style={{
      position:'fixed', left, top, zIndex:1000,
      width:W, background:'var(--paper, #fff)', border:'1px solid var(--line)',
      boxShadow:'0 8px 24px rgba(0,0,0,0.18)', padding:'12px 14px', pointerEvents:'none'
    }}>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:8}}>
        <span className="mono" style={{fontSize:10, color:'var(--muted)', letterSpacing:'0.14em', textTransform:'uppercase'}}>score breakdown</span>
        <span className="serif" style={{fontSize:18, color:tierColor}}>{Math.round(total)}<span style={{fontSize:11, color:'var(--muted)'}}>/{Math.round(maxSum)}</span></span>
      </div>
      {comps.map(c => {
        const pct = c.max > 0 ? Math.max(0, Math.min(1, c.points / c.max)) : 0;
        return (
          <div key={c.key} style={{marginBottom:8}}>
            <div style={{display:'flex', justifyContent:'space-between', fontSize:12, marginBottom:3}}>
              <span style={{color:'var(--ink)'}}>{c.label}</span>
              <span className="mono" style={{color:'var(--muted)', fontSize:11}}>
                {Number(c.points).toFixed(1)}/{Number(c.max).toFixed(0)}
              </span>
            </div>
            <div style={{height:4, background:'var(--line)', position:'relative'}}>
              <div style={{position:'absolute', left:0, top:0, bottom:0, width:`${pct*100}%`, background:tierColor}}/>
            </div>
            {c.detail && <div className="mono" style={{fontSize:10, color:'var(--muted)', marginTop:3, letterSpacing:'0.02em'}}>{c.detail}</div>}
          </div>
        );
      })}
    </div>
...
const FEATURE_META = {
  ...
  miklat:{he:'מקלט בבניין', en:'Building shelter', glyph:'▢'},
...
function Features({f}){
  // Render only features with positive evidence. Our scraper model is
  // positive-evidence-only: `false` means "not found in text", not
  // "confirmed absent" — so striking them through was misleading. We
  // show confirmed amenities; if nothing is confirmed, show an unobtrusive
  // "no amenities confirmed" hint so an empty row doesn't look broken.
  const items = Object.keys(FEATURE_META)
    .filter(k => !!f[k])
    .map(k => ({k, ...FEATURE_META[k]}));
  if(items.length === 0){
    return (
      <div className="mono" style={{fontSize:10, color:'var(--muted)', letterSpacing:'0.04em', opacity:0.7}}>
        no amenities confirmed
      </div>
    );
  }
  return (
...
function MiniCard({it}){
  return (
    <div style={{display:'grid', gridTemplateColumns:'130px 1fr auto', gap:14, padding:14, alignItems:'center'}}>
      ...
      <div style={{minWidth:0}}>
        <div className="mono" style={{fontSize:10, color:'var(--muted)', letterSpacing:'0.12em', textTransform:'uppercase'}}>{it.source} · posted {it.postedLabel || 'unknown'} · scraped {it.scrapedLabel || `${it.daysAgo}d ago`}</div>
...
function useScraperStatus(){
  const [data, setData] = useState({
    isPaused: false, isBlocked: false, scrapers: {}, bySource: {},
    lastRunSeconds: null, byCaptcha: null,
  });
  const [intervals, setIntervals] = useState({yad2:15, madlan:15, facebook:20});
  const [tick, setTick] = useState(0);
  const baseline = useRef({ts: Date.now(), seconds: {}, lastRun: null});

  const refresh = async () => {
    try {
      const [h, st, db] = await Promise.all([
        fetch('/api/health').then(r=>r.json()),
        fetch('/api/status').then(r=>r.json()),
        fetch('/api/db-stats').then(r=>r.json()),
      ]);
      const scrapers = h.scrapers || {};
      const secs = {};
      Object.keys(scrapers).forEach(k => { secs[k.toLowerCase()] = scrapers[k].seconds_since_scrape; });
      let lastRun = null;
      Object.values(scrapers).forEach(s => {
        if(s.seconds_since_scrape != null && (lastRun==null || s.seconds_since_scrape < lastRun)) lastRun = s.seconds_since_scrape;
      });
      baseline.current = {ts: Date.now(), seconds: secs, lastRun};
      setData({
        isPaused: !!st.is_paused,
        isBlocked: !!(st.captcha && st.captcha.status === 'WAITING_FOR_CAPTCHA'),
        scrapers: {},
        bySource: (db.database && db.database.by_source) || {},
        lastRunSeconds: lastRun,
        byCaptcha: st.captcha && st.captcha.status === 'WAITING_FOR_CAPTCHA' ? st.captcha : null,
      });
  ...
  useEffect(() => {
    fetch('/api/settings').then(r=>r.json()).then(s => {
      setIntervals({yad2: s.yad2_interval||15, madlan: s.madlan_interval||15, facebook: s.facebook_interval||20});
    }).catch(()=>{});
    refresh();
  ...
  const now = Date.now();
  const drift = Math.floor((now - baseline.current.ts)/1000);
  const liveLastRun = baseline.current.lastRun != null ? baseline.current.lastRun + drift : null;
  const scrapers = {};
  ['yad2','madlan','facebook'].forEach(k => {
    const base = baseline.current.seconds[k];
    const sec = base != null ? base + drift : null;
    scrapers[k] = {
      seconds_since: sec,
      interval_min: intervals[k],
      count: data.bySource[k] || 0,
    };
  });

  return {
    ...data,
    lastRunSeconds: liveLastRun,
    scrapers,
    refresh,
  };
}
...
function Dashboard({state, set}){
  ...
  useEffect(()=>{
    ...
    if(filters.city) {
      const cityList = filters.city.split(',').map(c=>c.trim()).filter(Boolean);
      cityList.forEach(c => params.append('city', c));
    }
    if(filters.neighborhood) {
      const hoodList = filters.neighborhood.split(',').map(c=>c.trim()).filter(Boolean);
      hoodList.forEach(n => params.append('neighborhood', n));
    }
    ...
    fetch('/api/listings?'+params.toString())
      ...
      .then(data=>{
        ...
        setStatusMap(prev => {
          const next = {...prev};
          data.forEach(l => { if(l.status && l.status !== 'unseen' && !next[l.id]) next[l.id] = l.status; });
          return next;
        });
        setLoading(false);
      })
  ...
  const filtered = useMemo(()=>{
    return listings.filter(l=>{
      if(filters.city) {
        const cityList = filters.city.split(',').map(c=>c.trim()).filter(Boolean);
        if(cityList.length > 0 && !cityList.includes(l.city)) return false;
      }
      if(filters.neighborhood) {
        const hoodList = filters.neighborhood.split(',').map(c=>c.trim()).filter(Boolean);
        if(hoodList.length > 0 && !hoodList.includes(l.neighborhood)) return false;
      }
      if(l.score < filters.minScore) return false;
      if(filters.mamad && !l.features.mamad) return false;
      if(filters.miklat && !l.features.miklat) return false;
  ...
  return (
    <div id="dashboard" style={{display:'grid', gridTemplateColumns:'280px 1fr', minHeight:'100vh'}}>
      <Sidebar filters={filters} setFilters={setFilters} state={state} set={set} status={status}/>
      <main style={{borderLeft:'1px solid var(--line)'}}>
        <TopBar state={state} set={set} status={status} onSettings={()=>setSettingsOpen(true)}/>
        {status.isBlocked && <CaptchaBanner status={status}/>}
        ...
        {state.view==='grid' ? (
          ...
          <MapView items={filtered} statusMap={statusMap} setLStatus={setLStatus}/>
        )}
        <DashFooter/>
      </main>
      {settingsOpen && <SettingsModal onClose={()=>{setSettingsOpen(false); status.refresh();}}/>}
    </div>
  );
}

function Sidebar({filters, setFilters, state, set, status}){
  const cities = (BOOTSTRAP.cities && BOOTSTRAP.cities.length) ? BOOTSTRAP.cities : [];
  const hoods = (BOOTSTRAP.neighborhoods && BOOTSTRAP.neighborhoods.length) ? BOOTSTRAP.neighborhoods : [];
  ...
  return (
    <aside style={{position:'sticky', top:0, alignSelf:'start', height:'100vh', overflowY:'auto', padding:'24px 22px', background:'var(--paper-2)'}}>
      ...
      <SidebarSection label="Filters">
        <Field label="Status">
          <Seg options={[['all','All'],['unseen','Unseen'],['viewed','Viewed'],['interested','Liked'],['contacted','Called']]} value={filters.status} onChange={v=>upd('status',v)}/>
        </Field>
        <Field label="Cities">
        ...
        <Field label="Neighborhoods">
          <div style={{marginBottom:8, display:'flex', gap:10}}>
            {(() => {
               const cityList = filters.city.split(',').map(x=>x.trim()).filter(Boolean);
               const filteredHoods = cityList.length > 0 ? hoods.filter(h => cityList.includes(h.city)) : hoods;
               const uniqueHoodNames = [...new Set(filteredHoods.map(h => h.name))];
               return (
                 <button onClick={() => upd('neighborhood', uniqueHoodNames.join(','))} style={{fontSize:11, color:'var(--ink)', textDecoration:'underline', background:'none', border:'none', padding:0, cursor:'pointer'}}>Select All</button>
               );
            })()}
        ...
        <Field label="Filter by">
          <div style={{display:'flex', flexWrap:'wrap', gap:6}}>
            {['mamad','miklat','parking','elevator','balcony'].map(k=>(
      ...
      <SidebarSection label="Scrapers" right={<Pill tone="accent" style={{fontSize:10, padding:'2px 8px'}}><span style={{width:5,height:5,borderRadius:'50%',background:'var(--accent-ink)'}}/> live</Pill>}>
        {['yad2','madlan','facebook'].map(src => {
          const s = (status && status.scrapers && status.scrapers[src]) || {};
          const captchaSrc = status && status.byCaptcha && status.byCaptcha.source;
          const isCaptcha = captchaSrc && captchaSrc.toLowerCase() === src;
          let next = '—';
          let warn = false;
          if(isCaptcha){ next = 'paused · captcha'; warn = true; }
          else if(s.interval_min && s.seconds_since != null){
            const remaining = Math.max(0, s.interval_min*60 - s.seconds_since);
            if(remaining === 0) next = 'due now';
            else {
              const mm = String(Math.floor(remaining/60)).padStart(2,'0');
              const ss = String(Math.floor(remaining%60)).padStart(2,'0');
              next = `in ${mm}:${ss}`;
            }
          } else if(s.interval_min){ next = `every ${s.interval_min}m`; }
          const label = src === 'yad2' ? 'Yad2' : src === 'madlan' ? 'Madlan' : 'Facebook';
          return <SourceRow key={src} name={label} next={next} count={s.count||0} warn={warn}/>;
        })}
      </SidebarSection>
...
function TopBar({state, set, status, onSettings}){
  const lastRun = status && status.lastRunSeconds != null ? status.lastRunSeconds : null;
  let lastStr = '—';
  if(lastRun != null){
    const mm = String(Math.floor(lastRun/60)).padStart(2,'0');
    const ss = String(Math.floor(lastRun%60)).padStart(2,'0');
    lastStr = `last scrape ${mm}:${ss} ago`;
  }
  const isBlocked = status && status.isBlocked;
  const isPaused = status && status.isPaused;
  const pillLabel = isBlocked ? 'Blocked · captcha' : (isPaused ? 'Paused' : 'Scraper running');
  const pillDotColor = isBlocked ? 'var(--danger)' : (isPaused ? 'var(--muted)' : 'var(--accent)');
  const btnLabel = isPaused ? 'Start' : 'Pause';
  const toggle = async () => {
    const action = isPaused ? 'start' : 'stop';
    try {
      await fetch('/api/scraper/' + action, {method:'POST'});
      if(status && status.refresh) status.refresh();
    } catch(e){ console.error(e); }
  };
  return (
    <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 28px', borderBottom:'1px solid var(--line)'}}>
      ...
      <div style={{display:'flex', gap:10, alignItems:'center'}}>
        <span className="mono" style={{fontSize:11, color:'var(--muted)'}}>{lastStr}</span>
        <Pill tone="ink"><span style={{width:6,height:6, borderRadius:'50%', background:pillDotColor}}/><span>{pillLabel}</span></Pill>
        <button onClick={onSettings} style={{padding:'7px 12px', border:'1px solid var(--line-2)', borderRadius:6, fontSize:13}}>Settings</button>
        <button onClick={toggle} style={{padding:'7px 12px', border:'1px solid var(--ink)', borderRadius:6, background:'var(--ink)', color:'var(--paper)', fontSize:13}}>{btnLabel}</button>
      </div>
    </div>
  );
}

function StatsStrip({status}){
  const bySrc = (status && status.bySource) || {};
  const total = (bySrc.yad2||0) + (bySrc.madlan||0) + (bySrc.facebook||0);
  const pct = (n) => total ? Math.round(n/total*100) : 0;
  const bars = total > 0 ? [
    {k:'Yad2', v: pct(bySrc.yad2||0), c:'var(--ink)'},
    {k:'Madlan', v: pct(bySrc.madlan||0), c:'var(--cool)'},
    {k:'FB', v: pct(bySrc.facebook||0), c:'var(--warn)'},
  ] : null;
  return (
    <div style={{display:'grid', gridTemplateColumns:'1.4fr 1fr 1fr 1fr', borderBottom:'1px solid var(--line)'}}>
      <StatCell label="Total listings" value={STATS.total.toLocaleString()} sub={`across ${Object.keys(bySrc).length||3} sources`} chart={<Spark data={STATS.trend7d}/>}/>
      <StatCell label="New today" value={STATS.newToday} sub="since midnight" tone="accent"/>
      <StatCell label="Hot deals · ≥80" value={STATS.hotDeals} sub="high deal score"/>
      <StatCell label="Avg deal score" value={STATS.avgScore} sub="source mix" bars={bars}/>
    </div>
  );
}
  ...
  if(filters.neighborhood) {
    ...
    hoodList.forEach(n => {
      active.push({
        ...
        label:n,
        clear:()=> {
          const next = hoodList.filter(x=>x!==n);
          setFilters(f=>({...f, neighborhood: next.join(',')}));
        }
      });
    });
  }
  ...
  return (
    <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 28px', borderBottom:'1px solid var(--line)', gap:12}}>
      ...
      <div style={{display:'flex', gap:6}}>
        <Seg options={[['grid','Grid'],['list','List'],['map','Map']]} value={state.view} onChange={v=>set({view:v})}/>
      </div>
    </div>
  );
}
    ...
    >
      <div style={{position:'relative'}}>
        ...
        <div style={{position:'absolute', top:12, left:12, right:12, display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
          ...
          <div style={{display:'flex', gap:6, flexDirection:'column', alignItems:'flex-end'}}>
            {it.isNew && (!status || status==='unseen') && <span className="mono" style={{fontSize:10, letterSpacing:'0.14em', background:'var(--accent)', color:'var(--accent-ink)', padding:'4px 8px', borderRadius:2}}>NEW</span>}
            {status==='viewed' && <span className="mono" style={{fontSize:10, letterSpacing:'0.14em', background:'var(--paper)', color:'var(--muted)', padding:'4px 8px', borderRadius:2, border:'1px solid var(--line-2)'}}>VIEWED</span>}
        ...
        <div style={{position:'absolute', right:-8, bottom:-34, background:'var(--paper)', border:'1px solid var(--line-2)', padding:6, borderRadius:'50%'}}>
          <ScoreDial score={it.score} size={compact?70:84} strokeWidth={compact?7:8} breakdown={it.scoreBreakdown}/>
        </div>
      </div>
      <div style={{padding: compact ? '14px 16px' : '22px 18px 18px', display:'flex', flexDirection:'column', gap:10, flex:1}}>
        ...
        <div style={{display:'flex', alignItems:'baseline', gap:8, flexWrap:'wrap'}}>
          ...
          <span className="mono" style={{fontSize:11, color:'var(--muted)'}}>· {(it.pricePerSqm||0).toFixed(1)} ₪/m²</span>
        </div>
        <div style={{display:'flex', gap:10, marginTop:-2}}>
          <Metric label="rooms" value={it.rooms}/>
          <Metric label="m²" value={it.sqm}/>
          <Metric label="floor" value={it.floor===0?'G':it.floor}/>
          <Metric label="scraped" value={it.scrapedLabel || `${it.daysAgo}d ago`}/>
          <Metric label="posted" value={it.postedLabel || 'unknown'}/>
        </div>
        <Features f={it.features}/>
        <div style={{marginTop:'auto', paddingTop:12, display:'flex', gap:6}}>
          <a href={it.url} target="_blank" rel="noopener"
             onClick={()=>{ if(!status || status==='unseen') setStatus('viewed'); }}
             style={{flex:1, padding:'10px 12px', textAlign:'center', border:'1px solid var(--ink)', background:'var(--ink)', color:'var(--paper)', borderRadius:2, fontSize:12, fontWeight:500, textDecoration:'none'}}>Open on {it.source}</a>
    ...
    <div style={{background:'var(--paper)'}}>
      ...
      {items.map(it=>(
        <div key={it.id} style={{display:'grid', gridTemplateColumns:'auto 1.5fr 1fr 1fr 1.4fr auto', gap:16, padding:rowPad, borderBottom:'1px solid var(--line)', alignItems:'center'}}>
          <ScoreDial score={it.score} size={54} strokeWidth={6} showLabel={false} breakdown={it.scoreBreakdown}/>
          <div>
            <div className="he" style={{fontSize:15, fontWeight:500, lineHeight:1.3}}>{it.he}</div>
            <div className="mono" style={{fontSize:10, color:'var(--muted)', marginTop:4, letterSpacing:'0.08em'}}>{it.source} · posted {it.postedLabel || 'unknown'} · scraped {it.scrapedLabel || `${it.daysAgo}d ago`} · {it.rooms} rm · {it.sqm} m² · floor {it.floor}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="mono" style={{fontSize:10, color: it.trend<0?'var(--ink)':'var(--muted)', marginTop:4}}>{(it.pricePerSqm||0).toFixed(1)} ₪/m² {it.trend<0 && <span style={{color:'oklch(0.6 0.16 150)'}}>· ↓ {Math.abs(it.trend)}%</span>}</div>
          </div>
          <Features f={it.features}/>
          <div style={{display:'flex', gap:6}}>
...
/* ---------- Map view ---------- */
function MapView({items, statusMap, setLStatus}){
  ...
  useEffect(() => {
    ...
    if(!mapInst.current) {
      mapInst.current = L.map(mapRef.current).setView([32.0853, 34.7818], 12);
    ...
    if (validItems.length > 0) {
      ...
      validItems.forEach(it => {
        const color = it.score >= 80 ? 'var(--accent)' : it.score >= 65 ? 'var(--ink)' : it.score >= 50 ? 'var(--warn)' : 'var(--muted)';
        const markerHTML = `<div style="background: ${color}; width: 14px; height: 14px; border-radius: 50%; border: 2px solid white; box-shadow: 0 1px 4px rgba(0,0,0,0.4);"></div>`;
        const icon = L.divIcon({ className: 'custom-map-marker', html: markerHTML, iconSize: [14, 14], iconAnchor: [7, 7] });
        
        const popupContent = `
          <div style="width: 220px; font-family: 'Inter', sans-serif;">
            <div style="font-weight: 600; font-size: 14px; margin-bottom: 4px; direction: rtl;">${it.he}</div>
            <div style="color: var(--muted); font-size: 11px; margin-bottom: 8px; direction: rtl;">${it.neighborhood}, ${it.city}</div>
            <div style="font-weight: 600; font-size: 16px; margin-bottom: 8px;">${formatILS(it.price)}</div>
            <div style="display: flex; gap: 8px; font-size: 11px; color: var(--ink-2); margin-bottom: 12px;">
              <span>${it.rooms} rm</span>
              <span>${it.sqm} m²</span>
              <span>Score: ${Math.round(it.score)}</span>
            </div>
            <a href="${it.url}" target="_blank" style="display: block; text-align: center; background: var(--ink); color: var(--paper); padding: 6px; text-decoration: none; border-radius: 4px; font-size: 12px; font-weight: 500;">View Listing</a>
          </div>
        `;

        const marker = L.marker([it.lat, it.lng], { icon }).bindPopup(popupContent);
        marker.addTo(map);
  ...
  return (
    <div style={{ padding: '24px 28px', background: 'var(--paper)', height: 'calc(100vh - 140px)' }}>
      <div ref={mapRef} style={{ width: '100%', height: '100%', borderRadius: '8px', border: '1px solid var(--line-2)', zIndex: 0 }}></div>
    </div>
  );
}
...
function TweaksPanel({state, set, active}){
  ...
  return (
    <div style={{
      ...
      <TweakRow label="View">
        <SegDark options={[['grid','Grid'],['list','List'],['map','Map']]} value={state.view} onChange={v=>set({view:v})}/>
      </TweakRow>
...
function CaptchaBanner({status}){
  const src = status.byCaptcha && status.byCaptcha.source || 'unknown';
  const elapsed = status.byCaptcha && status.byCaptcha.elapsed_minutes;
  return (
    <div style={{padding:'12px 28px', background:'color-mix(in oklch, var(--danger) 14%, var(--paper))', borderBottom:'1px solid var(--danger)', color:'var(--ink)', display:'flex', gap:12, alignItems:'center'}}>
      <span className="mono" style={{fontSize:11, letterSpacing:'0.14em', textTransform:'uppercase', background:'var(--danger)', color:'var(--paper)', padding:'3px 8px', borderRadius:2}}>CAPTCHA</span>
      <span style={{fontSize:13}}>Scraper <b>{src}</b> is waiting for a CAPTCHA to be solved{elapsed!=null?` · ${Number(elapsed).toFixed(1)} min elapsed`:''}. Solve it in the Chrome debug window (port 9222).</span>
    </div>
  );
}

function CitiesChipInput({value, onChange}){
  const [draft, setDraft] = useState('');
  const cities = (value || '').split(',').map(s=>s.trim()).filter(Boolean);
  const commit = () => {
    const v = draft.trim().replace(/,+$/,'');
    if(!v){ setDraft(''); return; }
    if(!cities.includes(v)) onChange([...cities, v].join(','));
    setDraft('');
  };
  const remove = (c) => onChange(cities.filter(x=>x!==c).join(','));
  return (
    <div style={{display:'flex', flexWrap:'wrap', gap:6, alignItems:'center', padding:'6px 8px', border:'1px solid var(--line-2)', borderRadius:6, background:'var(--paper)', minHeight:40}}>
      {cities.map(c => (
        <span key={c} style={{display:'inline-flex', alignItems:'center', gap:6, padding:'3px 4px 3px 10px', borderRadius:999, background:'var(--ink)', color:'var(--paper)', fontSize:12}}>
          <span className="he">{c}</span>
          <button type="button" onClick={()=>remove(c)} aria-label={`remove ${c}`} style={{color:'var(--paper)', opacity:0.8, fontSize:14, lineHeight:1, padding:'0 6px', background:'transparent'}}>×</button>
        </span>
      ))}
      <input value={draft}
        ...
        onKeyDown={e=>{
          ...
          else if(e.key==='Backspace' && !draft && cities.length){ remove(cities[cities.length-1]); }
        }}
        onBlur={commit}
        placeholder={cities.length?'add another city…':'type a city and press Enter'}
        style={{flex:1, minWidth:140, border:0, outline:0, background:'transparent', fontSize:13, padding:'4px 2px'}}/>
    </div>
  );
}
...
function SettingsModal({onClose}){
  ...
  useEffect(()=>{
    fetch('/api/settings').then(r=>r.json()).then(setForm).catch(e=>console.error(e));
  },[]);
  ...
  const save = async () => {
    ...
    try {
      const body = {
        ...
        max_rooms: form.max_rooms === '' || form.max_rooms == null ? null : Number(form.max_rooms),
        min_size_sqm: Number(form.min_size_sqm),
        require_mamad: !!form.require_mamad,
        yad2_interval: Number(form.yad2_interval),
        madlan_interval: Number(form.madlan_interval),
        facebook_interval: Number(form.facebook_interval),
        min_deal_score: Number(form.min_deal_score),
      };
  ...
  const saveAndRestart = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/settings', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body)});
      if(!r.ok) throw new Error('save failed');
      const r2 = await fetch('/api/scraper/restart', {method:'POST'});
      if(!r2.ok) throw new Error('restart failed');
      onClose();
    } catch(e){ console.error(e); alert('Failed to save settings or restart scraper'); }
    finally { setSaving(false); setRestarting(false); }
  };
  const row = (label, child) => (
    <div style={{display:'grid', gridTemplateColumns:'180px 1fr', alignItems:'center', gap:12, padding:'8px 0'}}>
      <label style={{fontSize:12, color:'var(--muted)'}}>{label}</label>
      {child}
    </div>
  );
  const input = (k, props={}) => (
    <input value={form[k]==null?'':form[k]} onChange={e=>upd(k,e.target.value)} {...props}
      style={{padding:'8px 10px', border:'1px solid var(--line-2)', borderRadius:6, background:'var(--paper)', fontSize:13, width:'100%'}}/>
  );
  return (
    <div onClick={onClose} style={{position:'fixed', inset:0, background:'rgba(14,15,12,0.55)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'var(--paper)', border:'1px solid var(--line-2)', borderRadius:8, width:560, maxHeight:'88vh', overflowY:'auto', padding:'22px 26px'}}>
        <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14}}>
          <span className="serif" style={{fontSize:24}}>Settings</span>
        ...
        {!form ? (
          ...
          <div>
            {row('Cities', <CitiesChipInput value={form.cities} onChange={v=>upd('cities', v)}/>)}
            {row('Max price (₪)', input('max_price', {type:'number'}))}
            {row('Min rooms', input('min_rooms', {type:'number', step:'0.5'}))}
            {row('Max rooms', input('max_rooms', {type:'number', step:'0.5'}))}
            {row('Min size (m²)', input('min_size_sqm', {type:'number'}))}
            {row('Require mamad', (
              <input type="checkbox" checked={!!form.require_mamad} onChange={e=>upd('require_mamad', e.target.checked)}/>
            ))}
            {row('Yad2 interval (min)', input('yad2_interval', {type:'number'}))}
            {row('Madlan interval (min)', input('madlan_interval', {type:'number'}))}
            {row('Facebook interval (min)', input('facebook_interval', {type:'number'}))}
            {row('Min deal score (notify)', input('min_deal_score', {type:'number'}))}
            <div style={{display:'flex', justifyContent:'flex-end', gap:8, marginTop:18}}>
              <button onClick={onClose} style={{padding:'8px 14px', border:'1px solid var(--line-2)', borderRadius:6, fontSize:13}}>Cancel</button>
              <button onClick={save} disabled={saving} style={{padding:'8px 14px', border:'1px solid var(--line-2)', borderRadius:6, fontSize:13, opacity:saving?0.6:1}}>{saving?'Saving…':'Save Only'}</button>
              <button onClick={saveAndRestart} disabled={saving} style={{padding:'8px 14px', border:'1px solid var(--accent)', borderRadius:6, background:'var(--accent)', color:'var(--accent-ink)', fontSize:13, fontWeight:600, opacity:saving?0.6:1}}>{restarting?'Restarting…':(saving?'Saving…':'Save & Restart Scraper')}</button>
            </div>
          </div>
        )}
      </div>
